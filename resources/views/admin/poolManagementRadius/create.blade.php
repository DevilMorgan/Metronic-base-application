...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.poolManagementRadiu.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.pool-management-radius.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="radius_pool_name">{{ trans('cruds.poolManagementRadiu.fields.radius_pool_name') }}</label>
                <input class="form-control {{ $errors->has('radius_pool_name') ? 'is-invalid' : '' }}" type="text" name="radius_pool_name" id="radius_pool_name" value="{{ old('radius_pool_name', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ip_ranges') ? 'is-invalid' : '' }}" type="text" name="ip_ranges" id="ip_ranges" value="{{ old('ip_ranges', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('radius_ip_nas') ? 'is-invalid' : '' }}" name="radius_ip_nas_id" id="radius_ip_nas_id">
                    @foreach($radius_ip_nas as $id => $radius_ip_nas)
                        <option value="{{ $id }}" {{ old('radius_ip_nas_id') == $id ? 'selected' : '' }}>{{ $radius_ip_nas }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\PoolManagementRadiu::IP_RADIUS_STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('ip_radius_status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="ip_radius_status_{{ $key }}" name="ip_radius_status" value="{{ $key }}" {{ old('ip_radius_status', '') === (string) $key ? 'checked' : '' }}>
