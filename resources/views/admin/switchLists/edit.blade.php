...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.switchList.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.switch-lists.update", [$switchList->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="switch_name">{{ trans('cruds.switchList.fields.switch_name') }}</label>
                <input class="form-control {{ $errors->has('switch_name') ? 'is-invalid' : '' }}" type="text" name="switch_name" id="switch_name" value="{{ old('switch_name', $switchList->switch_name) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('switch_ip_address') ? 'is-invalid' : '' }}" type="text" name="switch_ip_address" id="switch_ip_address" value="{{ old('switch_ip_address', $switchList->switch_ip_address) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('switch_marka') ? 'is-invalid' : '' }}" name="switch_marka" id="switch_marka">
                    ...
                    @foreach(App\Models\SwitchList::SWITCH_MARKA_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('switch_marka', $switchList->switch_marka) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('switch_port_sayisi') ? 'is-invalid' : '' }}" type="text" name="switch_port_sayisi" id="switch_port_sayisi" value="{{ old('switch_port_sayisi', $switchList->switch_port_sayisi) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('switch_status') ? 'is-invalid' : '' }}" name="switch_status" id="switch_status">
                    ...
                    @foreach(App\Models\SwitchList::SWITCH_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('switch_status', $switchList->switch_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('switch_api_izin') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="switch_api_izin" id="switch_api_izin" value="1" {{ $switchList->switch_api_izin || old('switch_api_izin', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('switch_notlar') ? 'is-invalid' : '' }}" type="text" name="switch_notlar" id="switch_notlar" value="{{ old('switch_notlar', $switchList->switch_notlar) }}">
