...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.nasManager.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.nas-managers.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('nas_sube') ? 'is-invalid' : '' }}" name="nas_sube_id" id="nas_sube_id">
                    @foreach($nas_subes as $id => $nas_sube)
                        <option value="{{ $id }}" {{ old('nas_sube_id') == $id ? 'selected' : '' }}>{{ $nas_sube }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nas_name') ? 'is-invalid' : '' }}" type="text" name="nas_name" id="nas_name" value="{{ old('nas_name', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nas_ip_address') ? 'is-invalid' : '' }}" type="text" name="nas_ip_address" id="nas_ip_address" value="{{ old('nas_ip_address', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nas_username') ? 'is-invalid' : '' }}" type="text" name="nas_username" id="nas_username" value="{{ old('nas_username', 'admin') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('nas_marka_model') ? 'is-invalid' : '' }}" name="nas_marka_model" id="nas_marka_model">
                    ...
                    @foreach(App\Models\NasManager::NAS_MARKA_MODEL_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('nas_marka_model', 'Mikrotik') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nas_ssh_port') ? 'is-invalid' : '' }}" type="number" name="nas_ssh_port" id="nas_ssh_port" value="{{ old('nas_ssh_port', '22') }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nas_api_port') ? 'is-invalid' : '' }}" type="number" name="nas_api_port" id="nas_api_port" value="{{ old('nas_api_port', '8728') }}" step="1">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('nas_permission_yonet') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="nas_permission_yonet" id="nas_permission_yonet" value="1" {{ old('nas_permission_yonet', 0) == 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('nas_status') ? 'is-invalid' : '' }}" name="nas_status" id="nas_status">
                    ...
                    @foreach(App\Models\NasManager::NAS_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('nas_status', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
